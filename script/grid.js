const sheetList = document.querySelector('.sheet-list'); // it containes all the sheets
const firstSheet = document.querySelector('.sheet');

///   =========  create sheet and make it active ====== /

const setUIelement = (sheetDB) => {
...
const addNewSheetToSheetList = () => {
  ...
  newSheet.addEventListener('dblclick', (e) => {
    e.preventDefault();
...
firstSheet.addEventListener('dblclick', (e) => {
  e.preventDefault();
  deleteSheets(firstSheet);
});
addBtn.addEventListener('click', addNewSheetToSheetList);
// ====== end of create sheet or make active sheet =========/

// =========  cell event => click , blur , kedown ========/
const allCell = document.querySelectorAll('.grid .cell');
for (let i = 0; i < allCell.length; i++) {
  allCell[i].addEventListener('click', () => {
