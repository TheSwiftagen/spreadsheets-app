...
for (let i = 0; i < allCell.length; i++) {
  ...
  allCell[i].addEventListener('blur', function () {
    ...
    if (cellObject.formula) {
      // agr koi formula change kia h toh phle wala remove kro
...
formulaBar.addEventListener('keydown', function (e) {
  if (e.key == 'Enter' && formulaBar.value) {
    ...
    if (currentFormula != cellObject.formula) {
      //agr formula formula bar s match nhi hota h tb cellobject s vo phlr wala formula remove kro
